...
open class LBXScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate {
    ...
    /**
     ...
     - returns:
     */
    init( videoPreView:UIView,objType:[AVMetadataObject.ObjectType] = [AVMetadataObject.ObjectType.qr],isCaptureImg:Bool,cropRect:CGRect=CGRect.zero, devicePostion: AVCaptureDevice.Position? = nil, success:@escaping ( ([LBXScanResult]) -> Void) )
    {
        do{
            if let postion = devicePostion, let dv = LBXScanWrapper.getCamera(postion: postion) {
                device = dv
            }
    ...
    func start()
    {
        if !session.isRunning
        {
            ...
            #if DEBUG
//            Log.shareInstance.log(message: "session.startRunning: \(session.isRunning)")
            #endif
        }
    }
    func stop()
    {
        if session.isRunning
        {
            ...
            #if DEBUG
//            Log.shareInstance.log(message: "session.stopRunning: \(session.isRunning)")
            #endif
        }
    }
...
//        return floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_8_0
    ...
    static public func createCode( codeType:String, codeString:String, size:CGSize,qrColor:UIColor,bkColor:UIColor,logo:UIImage? = nil )->UIImage?
    {
        //if #available(iOS 8.0, *)
        ...
        if let logoImage = logo{
            return LBXScanWrapper.addImageLogo(srcImg: codeImage!, logoImg: logoImage, logoSize: CGSize(width: size.width/4, height: size.height/4))
        }
        return codeImage        
...
//        Log.shareInstance.log(message: "LBXScanWrapper Deinit....")
    }
...
extension LBXScanWrapper {
    static func getCamera(postion: AVCaptureDevice.Position) -> AVCaptureDevice? {
    ...
    func swapFrontAndBackCameras() {
        if let input = input {
            ...
            if input.device.position == .front {
                newDevice = LBXScanWrapper.getCamera(postion: AVCaptureDevice.Position.back)
            } else {
                newDevice = LBXScanWrapper.getCamera(postion: AVCaptureDevice.Position.front)
            }
