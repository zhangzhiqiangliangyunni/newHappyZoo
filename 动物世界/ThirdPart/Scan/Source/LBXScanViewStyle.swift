...
public struct LBXScanViewStyle
{
 ...
 /// 是否需要绘制扫码矩形框，默认YES
    ...
    */
    ...
    mutating func setAliStyle() {
        guard let windowSize = UIApplication.shared.keyWindow?.frame.size else {return}
        centerUpOffset = -10
        xScanRetangleOffset = (windowSize.width - 360)/2
        ...
        photoframeLineW = 2.0
        photoframeAngleW = 100
        photoframeAngleH = 100
        isNeedShowRetangle = false
        anmiationStyle = LBXScanViewAnimationStyle.NetGrid
        animationImage = UIImage(named: "CodeScan.bundle/qrcode_scan_full_net")
    }
}
